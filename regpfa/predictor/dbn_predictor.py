...
class DBN:
    def __init__(self, log, states_k):
        ...
        self.numberofcontextvariables = 2
        self.prior = []
        self.obsmat = []
        self.transcube = []
        self.log = log
    ...
    def set_randomdistributions(self):
        ...
        self.prior = prior
        obsmat = np.random.dirichlet(np.ones(self.numberofsymbols),(self.numberofstates,self.numberofcontextvariables)) #TODO: Adjust obsmat for DBN
        self.obsmat = obsmat
    ...
    def predictProbability(self, trace):
        ...
        for i in range(0, self.log.get_numberOfUniqueSymbols() - 1):
            ...
            for j in range(0, self.numberofstates - 1):
                symbolDistributionTrace[i] = symbolDistributionTrace[i] + stateDistributionTrace[j] * \
                         self.obsmat[j][i]
    ...
    def scoreAccuracy(self):
        ...
        for trace in self.log.traces:
            if trace.get_lasteventfrompath() == self.predict(trace.get_pathwithoutlastevent()):
                result.append(1.0)
            else:
                result.append(0.0)
